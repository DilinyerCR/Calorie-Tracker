...
const Form = ({dispatch, state} : FormProps) => {
    ...
    const handleChange = (e : React.ChangeEvent<HTMLInputElement> | React.ChangeEvent<HTMLSelectElement>) => {
        const isumberField = ['category', 'calories'].includes(e.target.id)

        setActivity({
            ...activity,
            [e.target.id]: isumberField ? +e.target.value : e.target.value
        })
    }
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setActivity(initialState) //En caso de que se repitan los "id", cambiar esta linea de codigo (video 148 min:2:30)
    }
